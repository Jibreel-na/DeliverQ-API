...
const transport = nodemailer.createTransport(config.emailInfo.smtp);
/* istanbul ignore next */
if (config.env !== 'test') {
  transport
    .verify()
    .then(() => logger.info('Connected to info email server'))
    .catch(() => logger.warn('Unable to connect to email server. Make sure you have configured the SMTP options in .env ' + JSON.stringify(config.emailInfo.smtp)));
}

const sendEmail = async (to, subject, html) => {
  const msg = { from: config.emailInfo.from, to, subject, html, sender: 'DeloverQ' };
...
const BookingCreateEmailByGen = async (to, username, subject, heading, text, link, Tag) => {
  ...
  const msg = { from: config.emailInfo.from, to, subject, html, text, sender: 'DeloverQ' };
  await transport.sendMail(msg);
};
