...
const createUser = {
  body: Joi.object().keys({
    ...
    cities: Joi.when('role', { is: company, then: Joi.array().items(Joi.string().custom(objectId)).min(0), otherwise: Joi.array().items(Joi.string().custom(objectId)).default([]).min(0) }),
